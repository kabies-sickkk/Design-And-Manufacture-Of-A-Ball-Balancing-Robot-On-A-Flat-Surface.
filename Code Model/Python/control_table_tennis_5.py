        ...
        for r in results:
            for box in r.boxes:
                ...
                if center[0] is not None and center[1] is not None:
                    deltaX, deltaY = int(center[0]), int(center[1])
                    devdeltaXY = float(deltaY / deltaX)
                    theta = math.atan(devdeltaXY)
                    detadegree = theta * (180 / math.pi)
                    servoangle = theta * (180 / 15)
                    pwm_ser = servoangle * (635 - 150) / 180

                    # Append pwm_ser to the list and apply moving average filter
                    pwm_ser_values.append(pwm_ser)
                    smoothed_pwm_ser = moving_average(pwm_ser_values, 5)  # Smoothing window size of 5

                    # Send the center coordinates and smoothed PWM value to Arduino
                    data = f"{center[0]},{center[1]},{int(smoothed_pwm_ser)}\n"
