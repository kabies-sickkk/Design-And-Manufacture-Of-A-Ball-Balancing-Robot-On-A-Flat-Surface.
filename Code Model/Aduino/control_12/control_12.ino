...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    int servo0SignalUp = NEUTRAL - OFFSET0 - pwm_ser;
    int servo5SignalUp = NEUTRAL + OFFSET5 + pwm_ser;

    // int servo0SignalDown = NEUTRAL - OFFSET0 + pwm_ser;
    // int servo5SignalDown = NEUTRAL + OFFSET5 - pwm_ser;

    int servo1SignalUp = NEUTRAL + pwm_ser;
    int servo2SignalUp = NEUTRAL - pwm_ser;
    int servo3SignalUp = NEUTRAL + pwm_ser;
    int servo4SignalUp = NEUTRAL - pwm_ser;
    ...
    if (x >= 260 && x <= 390) {
      if (y < 215) {
        ...
        servo1 = servo1SignalUp = NEUTRAL + pwm_ser;
        servo2 = servo2SignalUp = NEUTRAL - pwm_ser;
        servo3 = servo3SignalUp = NEUTRAL + pwm_ser;
        servo4 = servo4SignalUp = NEUTRAL - pwm_ser;
      } else if (y > 280) {
    ...
    } else if (x > 390 && y <= y0) {  // Nếu x > 390 và y <= 240
      if (vector1Magnitude < vector2Magnitude) {
        servo0 = NEUTRAL - OFFSET0;
        servo5 = NEUTRAL + OFFSET5;
        servo3 = NEUTRAL + servoDiff;
    ...
    } else if (x < 260 && y > y0) {  // Nếu x < 260 và y > 240
      if (vector1Magnitude < vector2Magnitude) {
        servo0 = NEUTRAL - OFFSET0;
        servo5 = NEUTRAL + OFFSET5;
        servo1 = NEUTRAL + servoDiff;
